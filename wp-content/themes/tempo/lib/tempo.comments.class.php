        ...
        switch ( $comment -> comment_type ) {
            case '' : {
                ...
                comment_reply_link( array_merge( $args , array(
                    'reply_text'    => esc_html__( 'Reply', 'tempo' ),
                ...
                if( $comment -> user_id == $post -> post_author ){
                    echo '<span class="tempo-author-tag">' . esc_html__( 'Author' , 'tempo' ) . '</span>';
                }
                ...
                echo sprintf( esc_html__( 'Posted on %1$s %2$s', 'tempo' ), '<span class="tempo-comment-titme">' . get_comment_date( esc_attr( get_option( 'time_format' ) ), $comment -> comment_ID ) . '</span>', '<span class="tempo-comment-date">' . get_comment_date( esc_attr( get_option( 'date_format' ) ) , $comment -> comment_ID ) . '</span>' );
                echo '</time>';
                ...
                if ( $comment -> comment_approved == '0' ) {
                    ...
                    esc_html_e( 'Your comment is awaiting moderation.' , 'tempo' );
                    echo '</em>';
                }
